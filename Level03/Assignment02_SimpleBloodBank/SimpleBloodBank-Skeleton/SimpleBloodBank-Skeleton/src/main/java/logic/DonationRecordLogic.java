...
//            person = parameterMap.get(ADMINISTRATOR)[0];
//            validator.accept(person, 100);
//        }
        
//        String donation = null;
//        if (parameterMap.containsKey(DONATION_ID)) {
//            donation = parameterMap.get(ADMINISTRATOR)[0];
//            validator.accept(donation, 100);
//        }
        Date created;
        try {
            //valided by the convertStringToDateTime() inherited from parent GenericLogic.java
            created = convertStringToDateTime(parameterMap.get( CREATED )[ 0 ]);
        }
        catch (ValidationException e) {
            created = new Date(); //if parsing failed, use current Date
        }
        ...
        if (!tested.equalsIgnoreCase(testedb.toString())) {
