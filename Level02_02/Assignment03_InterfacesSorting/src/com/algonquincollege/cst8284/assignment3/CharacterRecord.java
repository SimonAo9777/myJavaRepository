...
/**
 * This class contains the data for a CharacterRecord.
 * 
 * @author Simon Ao<br>
 ...
 * 
 * <p>
 * This class contains the data for a CharacterRecord. The fields have basic
 * data validation in each of the mutator methods (setters).
 * </p>
 * <p>
 * - Every set method validates the data passed into it.<br />
 * - throws an IllegalArgumentException with a message containing the field
 * name, and reason for the exception being thrown.<br />
 * - None of the String fields should be set with text containing a comma
 * character e.g. ','<br />
 * - The field named "name" cannot exceed 50 characters in length. (The other
 * String fields do not have any limits to length.)<br />
 * - attackChance1, and attackChance2 cannot be negative, cannot exceed 100.
 * Note: The sum of attackChance1 and attackChance2 can exceed 100, but each
 * separate field cannot.<br />
 */
public class CharacterRecord {
	...
	public void setName(String name) {
		if (name != null && name.length() > 50) {
			throw new IllegalArgumentException("name cannot exceed 50 characters in length");
		}
		this.name = name;
	}
	...
	public void setAttackChance1(int attackChance1) {
		if (attackChance1 < 0 || attackChance1 > 100) {
			throw new IllegalArgumentException("attackChance1 cannot be negative, cannot exceed 100");
		}
	...
	public void setAttackChance2(int attackChance2) {
		if (attackChance2 < 0 || attackChance2 > 100) {
			throw new IllegalArgumentException("attackChance2 cannot be negative, cannot exceed 100");
		}
	...
	/**
	 * Provides a comma-separated-value String representation. There is no
	 * formatting of fields, and there is no ending line terminator. The order of
	 * the fields returned is:
	 * name,health,strength,attackDamage,attackChance1,attackChance2,
	 * attackType1,attackType2,defense @ return a comma-separated-value String
	 * representation.
	 */
	@Override
	public String toString() {
		String format = "%s,%s,%s,%s,%d,%d,%s,%s,%s";
		return String.format(format, getName(), getHealth(), getStrength(), getAttackDamage(), getAttackChance1(),
				getAttackChance2(), getAttackType1(), getAttackType2(), getDefense());
