...
//TODO: fix the JPQL for "BloodBank.findByName", "BloodBank.isDuplicate" bb.name = test
//@ThisAnnotationDoesNotExistSoYouMustFixThisBeforeTheProjectCanDeploy
@NamedQuery( name = BloodBank.SPECIFIC_BLOODBANKS_QUERY_NAME, query = "SELECT distinct b FROM BloodBank b left JOIN FETCH b.donations")//this will not be a problem 
@NamedQuery( name = BloodBank.IS_DUPLICATE_QUERY_NAME, query = "SELECT bb FROM BloodBank bb")//remove the where to test
...
public abstract class BloodBank extends PojoBase implements Serializable {
   ...
   // @OneToMany(cascade = CascadeType.ALL,orphanRemoval=true)
	@OneToMany( cascade = { CascadeType.PERSIST, CascadeType.MERGE, CascadeType.REFRESH,
			CascadeType.REMOVE }, orphanRemoval = true, mappedBy = "bank")
