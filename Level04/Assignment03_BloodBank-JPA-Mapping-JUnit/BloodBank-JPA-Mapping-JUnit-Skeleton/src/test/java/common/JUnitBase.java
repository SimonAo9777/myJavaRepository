 ...
 * @version Mar 12, 2021
 * @author (update)Simon Ao #040983402
 */
public class JUnitBase {
	...
	protected static void deleteAllData() {
		...
		//The following is that other table should refer to the deleted table through a foreign key.
	...
	protected static < T> List< T> getAll( EntityManager em, Class< T> clazz) {
		...
		CriteriaQuery<T> cq = cp.createQuery(clazz);
	...
	protected static < T, R> T getWithId( EntityManager em, Class< T> clazz, Class< R> classPK,
		...
		// method signature is just a suggestion it can be modified if need be.
		CriteriaBuilder cb = em.getCriteriaBuilder();
		// As long as we need to find the number of rows, we can create a query
		CriteriaQuery<T> query = cb.createQuery(clazz);		
		Root<T> root = query.from(clazz);
		query.select(root);
		query.where(cb.equal(root.get(sa), cb.parameter(classPK, "id")));
